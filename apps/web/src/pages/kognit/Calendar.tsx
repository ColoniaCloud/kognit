...
export const CalendarScreen = () => (
  <div className="min-h-full bg-background pb-28">
    ...
    <div className="px-6 mt-5 flex items-end justify-between">
      ...
      <button className="flex items-center gap-1.5 bg-gradient-primary text-primary-foreground text-xs font-bold px-3.5 py-2 rounded-full shadow-soft">
    ...
    <div className="px-6 mt-3 space-y-3">
      {notes.map((n, i) => (
        <div key={i} className="p-4 rounded-2xl bg-card shadow-soft">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-9 h-9 rounded-xl bg-secondary flex items-center justify-center text-lg">{n.mood}</div>
              <div>
                <p className="text-[10px] text-muted-foreground font-semibold">{n.time}</p>
    ...
    <div className="mx-6 mt-4 p-4 rounded-2xl bg-card shadow-soft border border-dashed border-border">
      ...
      <div className="mt-3 flex items-center gap-2">
        ...
        <button className="ml-auto w-9 h-9 rounded-xl bg-gradient-primary text-primary-foreground flex items-center justify-center shadow-soft">
          <Plus size={16} />
        </button>
      </div>
    </div>
